...
wire deviceDDR3					= (busaddress[31:28]==4'b0000) ? 1'b1 : 1'b0;							// 0x00000000 - 0x0FFFFFFF (256Mbytes-128Kbytes due to overlap)
...
switchfifo DeviceSwitchStates(
	...
	.din(switchdatain),
	.wr_en(switchwe),
	.wr_clk(uartbase), // Use slowest clock we have (uartbase==10Mhz)
	// Out
	.empty(switchempty),
	.dout(switchdataout),
	.rd_en(switchre),
...
wire infifoempty, infifovalid, uartbyteavailable;
...
// CPU can NOT execute code from this area, only read or write to it
// 0x10000000
// -----------------------------------------------------------------------
...
sysmem GraphicsRAM(
	...
	.ena(deviceGRAM ? (reset_n & (busre | (|buswe))) : 1'b0),	// Unit enabled only when not in reset and reading or writing, and not while reading instructions 
	// GPU DMA port
...
always_comb begin
	...
	if (deviceGRAM & (|buswe)) videomemdatain = busdatain; else videomemdatain = 32'd0;

	// For audio memory writes from CPU
	if (deviceARAM & (|buswe)) audiomemdatain = busdatain; else audiomemdatain = 32'd0;
	...
	endcase*/
	outfifoin = busdatain[7:0]; // Always 8 bits - make sure to enforce in code
	...
	endcase*/
	sdwq_datain = busdatain[7:0]; // Always 8 bits - make sure to enforce in code
